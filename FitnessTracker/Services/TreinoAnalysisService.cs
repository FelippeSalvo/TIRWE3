...
/// - Volume total (séries como proxy, já que exercícios vêm do catálogo)
...
/// Score final: soma dos 4 critérios, cada um contribuindo até 25 pontos (total 0-100).
/// </summary>
public class TreinoAnalysisService : ITreinoAnalysisService
{
    ...
    private const double MaxProporcaoGrupo = 0.5;

    public TreinoAnalysisService(ITreinoRepository treinoRepository, IExerciseRepository exerciseRepository)
    {
    ...
    public async Task<TreinoAnaliseDto?> AnalisarAsync(string idTreino, string usuarioId)
    {
        ...
        var lista = treino.ListaExercicios ?? new List<TreinoExercicioItem>();
        if (lista.Count == 0)
        {
            return new TreinoAnaliseDto
            {
                ...
                LogicaScore = "O score é a soma de 4 critérios (0-25 pts cada): volume total, frequência por grupo muscular, distribuição equilibrada e quantidade de séries. Treino vazio = 0."
            };
        }

        var exercises = await _exerciseRepository.GetByIdsAsync(lista.Select(e => e.ExerciseId));
        var exerciseById = exercises.ToDictionary(e => e.Id, StringComparer.OrdinalIgnoreCase);

        var validItems = lista.Where(item => exerciseById.ContainsKey(item.ExerciseId)).ToList();
        var totalSeries = validItems.Sum(item => item.Series < 1 ? 3 : item.Series);
        var volumeTotal = totalSeries * 100m; // proxy: séries * 100 (sem carga/rep no template)
        var grupos = validItems
            .Select(item => exerciseById[item.ExerciseId].GrupoMuscularPrincipal.Trim())
            .Where(g => !string.IsNullOrEmpty(g))
            .GroupBy(g => g, StringComparer.OrdinalIgnoreCase)
            .ToList();
        var quantidadeGrupos = grupos.Count;
        var maxExerciciosEmUmGrupo = quantidadeGrupos > 0 ? grupos.Max(g => g.Count()) : 0;
        var exercicioCount = validItems.Count;
        var proporcaoMaiorGrupo = exercicioCount > 0 ? (double)maxExerciciosEmUmGrupo / exercicioCount : 0;
        ...
        var logicaScore = "Score = soma de 4 critérios (cada um 0-25 pts): " +
            "(1) Volume total (proxy por séries); (2) Frequência: 4+ grupos = 25 pts; " +
    ...
    private static List<string> GerarSugestoes(decimal volumeTotal, int totalSeries, int quantidadeGrupos, double proporcaoMaiorGrupo, int pontosVolume, int pontosFrequencia, int pontosDistribuicao, int pontosSeries)
    {
        ...
        if (pontosVolume < 20 && volumeTotal < VolumeIdealMin)
            sugestoes.Add("Aumente o volume total do treino (mais séries ou mais exercícios).");
